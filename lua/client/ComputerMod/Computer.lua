...
function ComputerMod.getGameDiscTooltipDescription(game)
	...
	local description = ""
	if gameData then
		description = " <RGB:1,1,0.8> Titre: <RGB:1,1,1> <SPACE> " .. gameData.title
		description = description .. " <LINE> <RGB:1,1,0.8> Editeur: <RGB:1,1,1> <SPACE> " .. gameData.publisher
		--description = description .. " <LINE> <RGB:1,1,0.8> Parution:      <RGB:1,1,1> " .. gameData.date
		description = description .. " <LINE> <RGB:1,1,0.8> Genre: <RGB:1,1,1> <SPACE> " .. gameData.genre
	else
		description = " <RGB:1,0,0> Disque inconnu"
	end
	return description
end
...
function ComputerMod.getRetailDiscTooltipDescription(media)
	local description = ""
	description = " <RGB:1,1,0.8> Titre: <RGB:1,1,1> <SPACE> " .. media.title
	description = description .. " <LINE> <RGB:1,1,0.8> Auteur: <RGB:1,1,1> <SPACE>" .. media.author
	return description
end
...
function ComputerMod.setGameDiscData(disc, game)
	if disc and disc:getType() == "Disc_Game" then
		disc:setName("Disque de jeux: " .. game.title)
...
local function doComputerMenu(player, context, worldobjects, test)
	...
	if clickedSquare then
		...
		if computer then
			...
			local option = context:addOptionOnTop("Ordinateur de bureau");
			if ComputerMod.isComputerPowered(computer) then
				...
				option.toolTip.name = "Ordinateur de bureau"
				option.toolTip.description = " <RGB:1,0.3,0.3> Une source d'alimentation est requise!"
...
function ComputerMod.Events.OnBeforeInteractionMenuPoweredOptions(playerNum, computer, state, option, subContext)
	if not state then
		subContext:addOption("Allumer", playerNum, doToggleComputer, computer)
	end
end
...
function ComputerMod.Events.OnInteractionMenuPoweredOptions(playerNum, computer, state, option, subContext)
	if state then
		...
		local computerInfoOption = subContext:addOption("Informations Ordinateur");
		computerInfoOption.toolTip = ISToolTip:new();
		computerInfoOption.toolTip.name = "Informations Ordinateur";
		computerInfoOption.toolTip:setTexture("appliances_com_01_77");
		local currentDiscName = "aucun";
		if currentDisc and currentDisc.game then currentDiscName = currentDisc.game.title; end
		local description = " <RGB:1,1,0.8> Jeux installes: <RGB:1,1,1> <LINE> " .. tostring(#allInstalledGamesIds) .. "/" .. tostring(MaxGames) .. " <LINE><LINE>";
		description = description .. " <RGB:1,1,0.8> Disque actuel: <RGB:1,1,1> <LINE> " .. currentDiscName;
		computerInfoOption.toolTip.description = description;

		if currentDisc then
			if currentDisc.type == "Disc" then
				-- option = subContext:addOption("Jouer au jeu", playerNum, doPlayGame, computer, currentDisc.game)
			end
			...
			if currentDisc.type == "Disc_Game" then
				...
				if currentDisc.game then
					...
					option = subContext:addOption("Demarrer le Disque", playerNum, doPlayGame, computer, game)
					option.toolTip = ISToolTip:new()
					option.toolTip:setTexture("Item_Disc");
					option.toolTip.name = "Jouer " .. currentDisc.name
				...
				if currentDisc.game and not ComputerMod.isGameInstalled(computer, currentDisc.game) then
					option = subContext:addOption("Installer un Disque", playerNum, doInstallGame, computer, currentDisc.game)
					option.toolTip = ISToolTip:new()
					option.toolTip:setTexture("Item_Disc");
					option.toolTip.name = "Installer " .. currentDisc.name
					if #allInstalledGamesIds >= MaxGames then
						option.notAvailable = true;
						option.toolTip.description = " <RGB:1,0.3,0.3> Pas assez d'espace disponible pour installer ce disque.";
					else
						option.toolTip.description = ComputerMod.getGameDiscTooltipDescription(currentDisc.game)
					end
				end
			...
			if currentDisc.type == "Disc_Retail" then
				--local option = subContext:addOption("Lire le disque", playerNum, doPlayGame, computer)
				--option.toolTip = ISToolTip:new()
			...
			local option = subContext:addOption("Ejecter le Disque", playerNum, doEjectDisc, computer)
			option.toolTip = ISToolTip:new()
			option.toolTip:setTexture("Item_Disc");
			option.toolTip.name = "Ejecter " .. currentDisc.name

			if currentDisc.type == "Disc" then
				option.toolTip.description = " <RGB:1,0,0> Disque inconnu"
			end
		...
		-- Doesnt have a disc inserted
			...
			if not playerInv:contains("Disc") and not playerInv:contains("Disc_Game") and not playerInv:contains("Disc_Retail") then
				local option = subContext:addOption("Pas de Disque");
				option.notAvailable = true
			else
				local diskDriveOption = subContext:addOption("Inserer un Disque");
					...
					for i=0, items:size() - 1 do
						...
						if not discAdded:contains(displayName) then
							...
							if ComputerMod.isGameInstalled(computer, game) then
								displayName = displayName .. " (Installer)";
								description = description .. " <LINE> <LINE> (Disque deja installer)"
							end
							...
							discOption.toolTip.name = "Inserer " .. item:getName()
		...
		if #allInstalledGamesIds > 0 then
			...
			local playGameOption = subContext:addOption("Jouer au jeu");
			...
			for i = 1, #allInstalledGamesIds do
				...
				if game then
					...
					gameOption.toolTip.name = "Jouer " .. game.title
					gameOption.toolTip.description = ComputerMod.getGameDiscTooltipDescription(game)
				end
			end
			
			-- Uninstall Installed Discs
			local uninstallGameOption = subContext:addOption("Desinstaller le jeu")
			...
			for i=1, #allInstalledGamesIds do
				...
				if game then
					...
					uninstallGameOption.toolTip.name = "Desinstaller " .. game.title
...
function ComputerMod.Events.OnAfterInteractionMenuPoweredOptions(playerNum, computer, state, option, subContext)
	if state then
		subContext:addOption("Eteindre", playerNum, doToggleComputer, computer)
	end
end
